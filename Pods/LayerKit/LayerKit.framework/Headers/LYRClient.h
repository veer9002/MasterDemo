 ...
 */
- (nullable LYRMessage *)newMessageWithParts:(nonnull NSSet<LYRMessagePart *> *)messageParts options:(nullable LYRMessageOptions *)options error:(NSError * _Nullable * _Nullable)error;
...
/**
 @abstract Configures the set of MIME Types for `LYRMessagePart` objects that
   will be automatically downloaded upon synchronization.
 @discussion A value of `nil` indicates that all content is to be downloaded
   automatically and an empty `NSSet` indicates that no content should be.
   The default value is a set containing the MIME Type @"text/plain".
   Message parts belonging to latest messages will be auto-downloaded first.
 @note The MIMEType values can also be expressed with wildcards, such as:
   @c @"image/{asterisk}" which would auto-download any image type;
   or @c @"{asterisk}/{asterisk}+preview" which would tell the client to
   download any message part having the "+preview" extension in the MIMEType.
